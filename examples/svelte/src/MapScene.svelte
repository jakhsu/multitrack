<script lang="ts">
  import { useOpacities, useScrollProgress } from "@multitrack/svelte";
  ...
  function activeLocationIndex() {
    return LOCATIONS.findIndex(
      (name) => (opacities.current[`text-${name}`] ?? 0) > 0,
    );
  }
</script>
...
<div class="scene">
  ...
  <!-- Title -->
  {#if (opacities.current["title"] ?? 0) > 0}
    {@const o = opacities.current["title"] ?? 0}
    <div
      class="title-section"
      style:opacity={o}
  ...
  {#each [
    ...
    {#if (opacities.current[key] ?? 0) > 0}
      {@const o = opacities.current[key] ?? 0}
      <div
  ...
  <!-- Outro -->
  {#if (opacities.current["outro"] ?? 0) > 0}
    {@const o = opacities.current["outro"] ?? 0}
    <div
      class="outro-section"
      style:opacity={o}
      style:transform="translateY({(1 - o) * 20}px)"
  ...
  <div
    class="progress-bar"
    style:width="{progress.scrollPercentage * 100}%"
  ></div>
</div>
